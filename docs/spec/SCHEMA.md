...
| `2` | Z80 コアや BIOS・実機メモリ配置に依存する（`CALL` `OUT` `INP` `MON`） |
| `3` | 周辺機器・ファイル・プリンタ・PIC など、後回しでシーンへの影響が小さいもの |

迷ったら小さい番号を選ばず、`notes` に判断理由を書く。

`PEEK`/`POKE` は本来この基準では phase 2（実機メモリ配置に依存）に属するが、
実在作品の計測で「実機のメモリマップとは無関係な単なるバイト配列」としてしか
使われていないことが分かったため、例外的に phase 1 としている
（`basic_commands.yaml` の該当エントリ notes 参照）。
...
| `usage_report_3rd_party` | 第三者が実機を操作した**使用記録**（個人ブログ・紹介記事・動画など）に記載がある。マニュアル（編纂物）とは性格が違うため区別する。強さの位置づけは下記参照 |
| `emulator_compat` | ROM を実行しない互換実装で観測した。**原則として採用しない**（他人の推測を写すため） |
| `none` | 根拠なし。中間コード表に名前があるだけ、など |

`usage_report_3rd_party` は `manual_3rd_party` とは別物であり、序列は
`hardware` > `manual_official` > `emulator_rom` > `manual_3rd_party` /
`usage_report_3rd_party`（両者は優劣を付けずどちらも「実機で確認は取れているが
編纂・検証を経ていない」段階として並列に扱う。マニュアルは編纂物、使用記録は
実機を操作した結果の断片的な報告という性質の違いがあるだけで、片方が
もう片方より弱いとは限らない） > `emulator_compat` > `none`。
...
この原則は `usage_report_3rd_party` にも同様に適用する。複数の使用記録が
一致しても、それらが同じ一次情報（同じ動画の書き起こし、同じ掲示板の孫引き等）に
由来する疑いが拭えない限り根拠は強くならない。真に独立な観測者による使用記録が
複数一致した場合のみ、確度が上がったと扱ってよい（`manual_3rd_party` へ格上げは
しない。あくまで `usage_report_3rd_party` のまま確度が上がったという注記に留める）。
