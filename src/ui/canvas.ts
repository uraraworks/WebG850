 ...
 *
 * 【0.09→0.02 に変更】 実機ブラウザで見たユーザーから「きつすぎる、
 * ほとんど無いくらいが丁度いい」との判断（文字の下に縦帯が残像のように
 * 見えていた）。完全に0にはせず、よく見れば分かる程度に弱めた。
 */
const CROSSTALK_MAX_ALPHA = 0.02;
/** 上下端で帯を強調する範囲（行数）。GBの40/144を48ドットへ比例縮小し、視認性を見て丸めた。 */
const CROSSTALK_EDGE_BAND_ROWS = 3;
/** 上下端の帯強調そのものの最大アルファ。CROSSTALK_MAX_ALPHA と同じ比率で弱めた（0.03→0.007）。 */
const CROSSTALK_EDGE_BAND_ALPHA = 0.007;
