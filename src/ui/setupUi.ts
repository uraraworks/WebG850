...
//
// 【構造の切り出し（テスト検出力の穴を塞ぐための分離）】 以前はこの結線処理が
// `main.ts` の `main()` として書かれており、`main.ts` を import した時点で
// 即座に実行されてしまっていた（`main()` の末尾で呼んでいたため）。
// そのため「実際にユーザーが踏む結線（`onLoadProgram` 等のコールバックの配線）」を
// テストから一度も通せず、結線を壊す退行（例：`onLoadProgram` を
// `directMode!.loadProgram(program)` へ書き換えてしまう）があってもテストが
// 検出できなかった。結線処理だけをここへ `export` された関数として切り出し、
// `main.ts` は「読み込まれたら呼ぶだけ」のエントリポイントへ痩せさせる
// （振る舞いは変えず、構造の切り出しのみ）。
...
/** ページの DOM を引いて全結線を行う。`main.ts` から1回だけ呼ばれる（テストからも呼べる）。 */
